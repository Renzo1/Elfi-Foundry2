...
abstract contract TargetFunctions is BaseTargetFunctions, Properties, BeforeAfter {
    ...
    function orderFacet_cancelOrder(uint8 _requestIndex, uint16 _answer) public {
        ...
        vm.prank(address(this)); // prolly redundant
    ...
    */
    ...
    modifier callAuto(uint16 _answer) {
        _;
        // if condition for autoReducePositions is met
        // call autoReducePositions
    }
    
    function sortPositions(uint16 _answer) internal view returns (bytes32[] memory) {
    ...
    */
    ...
    function orderFacet_createOrderRequest(
        ...
        for(uint256 i = 0; i < oracles.length; i++) {
            if(oracles[i].token == params.marginToken) {
                params.triggerPrice = EchidnaUtils.clampBetween(uint256(_triggerPrice), uint256(oracles[i].maxPrice) / 5, uint256(oracles[i].maxPrice) * 5); 
        ...
        params.leverage = EchidnaUtils.clampBetween(uint256(_leverage), 0, MarketConfig.getMaxLeverage() * 2);
    ...
    function _createBatchOrders(
        ...
        */
        ...
        for (uint256 i = 0; i < _numOrders; i++) {
            ...
            params.leverage = EchidnaUtils.clampBetween(uint256(_orderMargin + 77), 0, MarketConfig.getMaxLeverage() * 2);
            params.executionFee = (ChainConfig.getPlaceIncreaseOrderGasFeeLimit() * tx.gasprice) + 10_000; // extra 10k to account for margin of error
            params.placeTime = block.timestamp;
    ...
    function positionFacet_createUpdateLeverageRequest(uint16 _answer, bool _isLong, bool _isNativeToken, uint8 _tokenIndex, uint96 _addMarginAmount, uint96 _leverage ) public {
        ...
        positionParamsHelper.leverage = EchidnaUtils.clampBetween(uint256(_leverage), 0, MarketConfig.getMaxLeverage() + 100);
    ...
    function stakeFacet_createMintStakeTokenRequest(
        ...
        params.requestTokenAmount = EchidnaUtils.clampBetween(uint256(_requestTokenAmount), params.walletRequestTokenAmount, uint256(_requestTokenAmount)); 
