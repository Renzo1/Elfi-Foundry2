...
library CalUtils {
    ...
    function mulRate(uint256 value, uint256 rate) internal pure returns (uint256) {
        return Math.mulDiv(value, rate, RATE_PRECISION);
    }

    function mulRate(int256 value, int256 rate) internal pure returns (int256) {
        return value.mul(rate).div(RATE_PRECISION.toInt256());
    }

    function divRate(uint256 value, uint256 rate) internal pure returns (uint256) {
        return Math.mulDiv(value, RATE_PRECISION, rate);
    }

    function divToPrecision(uint256 value1, uint256 value2, uint256 precision) internal pure returns (uint256) {
        return Math.mulDiv(value1, precision, value2);
    }

    function mulDiv(uint256 value1, uint256 value2, uint256 value3) internal pure returns (uint256) {
        return Math.mulDiv(value1, value2, value3);
    }

    function divToIntPrecision(int256 value1, int256 value2, int256 precision) internal pure returns (int256) {
        return value1.mul(precision).div(value2);
    }

    function mulSmallRate(uint256 value, uint256 rate) internal pure returns (uint256) {
        return Math.mulDiv(value, rate, SMALL_RATE_PRECISION);
    }

    function mulIntSmallRate(int256 value, int256 rate) internal pure returns (int256) {
        return value.mul(rate).div(SMALL_RATE_PRECISION.toInt256());
    }

    function divSmallRate(uint256 value, uint256 rate) internal pure returns (uint256) {
        return Math.mulDiv(value, SMALL_RATE_PRECISION, rate);
    }

    function quietAdd(uint256 value1, int256 value2) internal pure returns (uint256) {
        int256 result = value1.toInt256() + value2;
        return result <= 0 ? 0 : result.toUint256();
    }

    function tokenToUsd(uint256 tokenAmount, uint8 tokenDecimals, uint256 tokenPrice) internal pure returns (uint256) {
        return tokenAmount.mul(tokenPrice).mul(PRICE_TO_WEI).div(10 ** tokenDecimals);
    }

    function tokenToUsdInt(int256 tokenAmount, uint8 tokenDecimals, int256 tokenPrice) internal pure returns (int256) {
    ...
    function formatToTickSize(uint256 value, uint256 tickSize, bool roundUp) internal pure returns (uint256) {
    ...
    function diff(uint256 value1, uint256 value2) internal pure returns (uint256) {
