...
library InsuranceFund {
    ...
    function addFunds(address stakeToken, address token, uint256 amount) internal {
    ...
    function getFundsTokens(InsuranceFund.Props storage self) internal view returns (address[] memory) {
        return self.funds.keys();
    }

    function getTokenFee(InsuranceFund.Props storage self, address token) internal view returns (uint256) {
        (bool exists, uint256 preAmount) = self.funds.tryGet(token);
