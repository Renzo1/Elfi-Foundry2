...
library PermissionFlag {
    ...
    function load(bytes32 featureKey) internal pure returns(Props storage self){
    ...
    function isAllowed(bytes32 featureKey, address to) internal view returns (bool){
        Props storage permission = load(featureKey);
        return permission.allowSet.contains(to);
    }

    function isDenied(bytes32 featureKey, address to) internal view returns (bool) {
        Props storage permission = load(featureKey);
        return permission.denySet.contains(to);
    }

    function validDenied(bytes32 featureKey, address to) internal view {
        Props storage permission = load(featureKey);
